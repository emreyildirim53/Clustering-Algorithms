...
public class CCSystem extends JPanel {
    ...
    /* Draw a label */
    private void drawLabel(Graphics2D g2d, CCLabel label) {
    	Point p = translate(new Point2D.Double(label.x,  label.y));
    	g2d.setColor(label.color);	
    	g2d.setFont(new Font("default", Font.BOLD, 13));	
    	g2d.setColor(Color.BLACK);
    	g2d.drawString(label.label, ShiftWest(p.x-2, 1), ShiftNorth(p.y-2, 1));
    	g2d.drawString(label.label, ShiftWest(p.x-2, 1), ShiftSouth(p.y-2, 1));
    	g2d.drawString(label.label, ShiftEast(p.x-2, 1), ShiftNorth(p.y-2, 1));
    	g2d.drawString(label.label, ShiftEast(p.x-2, 1), ShiftSouth(p.y-2, 1));
    	g2d.setColor(label.color);
    	g2d.drawString(label.label, p.x-2, p.y-2);	
    }

    int ShiftNorth(int p, int distance) {return (p - distance);}
  	int ShiftSouth(int p, int distance) {return (p + distance);}
  	int ShiftEast(int p, int distance) {return (p + distance);}
  	int ShiftWest(int p, int distance) {return (p - distance);}
    /* Draw a polygon */
...
//    }
    ...
    public void paintComponent(Graphics g) {
        ...
        for (CCLabel l : labels) drawLabel(g2d, l);
    ...
    class mouseWheelListener implements MouseWheelListener {
        ...
        public void mouseWheelMoved(MouseWheelEvent e) {
            ...
            double zoomx = distX / 70.0 * units;
            double zoomy = distY / 70.0 * units;
